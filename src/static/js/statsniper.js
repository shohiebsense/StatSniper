...
window.StatSniper = {
  ...
  html: null,
  background: null,
  processorLabelsArray: null,
  storageLabelsArray: null,
  processorTriangle: null,
  storageTriangle: null,
  chart: null,
  ramLabelsArray: null,
  logoPage: null,
  contactsPage: null,
  ...
  currentMemory: null,
  currentMachineBitDepth: null,
  currentCPUBitDepth: null,
  currentDiskSwap: null,
  currentCpuName: null,
  currentOsName: null,
  currentTotalStorage: null,
  currentCpuCoreCount: null,
  kernel: null,
  processCount: null,
  availableRam: null,
  currentDiskCount: null,
  firstControl: null,
  secondControl: null,
  cloudLeft: null,
  cloudRight: null,
  ...
  InitializeDOMElements: function () {
    ...
    this.processesTbody = document.querySelector(".cpu-processes-table tbody");
  },
  AttachEventListeners: function () {
    this.firstControl.addEventListener("click", (event) =>
      this.ChangePage(event.target || event.srcElement)
    );
    this.secondControl.addEventListener("click", (event) =>
      this.ChangePage(event.target || event.srcElement)
    );
  },
  ShowCards: function () {
  ...
  SendAllInfoRequest: function () {
    window.StatSniper.AjaxRequest(
      window.StatSniper.XHR(),
      "GET",
      "/api/all",
      (response) => {
        ...
        window.StatSniper.UpdateProcesses(response.cpuProcesses);
        setTimeout(window.StatSniper.SendAllInfoRequest, 1000);
      }
    );
  },
  UpdateProcesses: function (response) {
      this.processesTbody.innerHTML = '';
      ...
      response.forEach((process) => {
        ...
        cpuUsageCell.textContent = process.cpuUsage.toFixed(2); // Round to 2 decimal places
        ...
        this.processesTbody.appendChild(row);
      });
  },
  UpdateInfo: function (response) {
    this.currentClockSpeed.innerHTML = this.FormatFrequencyMHz(
      response.systemInfo.processor.clockSpeed
    );
    this.freeClockSpeed.innerHTML =
      this.FormatFrequencyMHz(
        response.systemInfo.processor.clockSpeed *
          response.systemInfo.processor.coreCount,
        0
      ) +
      "/" +
      this.FormatFrequencyMHz(
        response.systemInfo.processor.clockSpeed *
          response.systemInfo.processor.coreCount -
          (response.usage.processor / 100) *
            (response.systemInfo.processor.clockSpeed *
              response.systemInfo.processor.coreCount),
        0
      );
    this.currentMemory.innerHTML = this.FormatBytes(
      response.systemInfo.machine.totalRam,
      2
    );
    this.currentCPUBitDepth.innerHTML = response.systemInfo.processor.bitDepth;
    this.currentTotalStorage.innerHTML = this.FormatBytes(
      response.systemInfo.storage.total
    );
    this.currentCpuCoreCount.innerHTML =
      response.systemInfo.processor.coreCount;
    this.currentDiskSwap.innerHTML = this.FormatBytes(
      response.systemInfo.storage.swapAmount
    );
    this.availableRam.innerHTML = this.FormatBytes(
      response.systemInfo.machine.availableRam,
      2
    );
  ...
  SetPageVisibility: function (newPage) {
    ...
    this.logoPage.style.visibility =
      newPage === 1 ? visibility[0] : visibility[1];
    this.contactsPage.style.visibility =
      newPage === 1 ? visibility[1] : visibility[0];
  },
  SetCloudAnimation: function (newPage) {
    ...
    this.cloudLeft.style.animation =
      animations[newPage - 1][0] + " 0.3s forwards";
    this.cloudRight.style.animation =
      animations[newPage - 1][1] + " 0.3s forwards";
  },
  SetControlOpacity: function (newPage) {
    const opacities = [0.5, 1];
    this.firstControl.style.opacity =
      newPage === 1 ? opacities[0] : opacities[1];
    this.secondControl.style.opacity =
      newPage === 1 ? opacities[1] : opacities[0];
  },
  BackgroundInitialization: function () {
    this.background = VANTA.FOG({
      el: "#background",
      blurFactor: 0.4,
      zoom: 1.5,
    });
  ...
  ChartInitialization: function () {
    ...
    this.chart = new Chart(
      ctx,
      Object.assign(
        this.html.getAttribute("theme") == "light" ? dataLight : dataDark,
        options
      )
    );

    processorRectangle.addEventListener("click", function (event) {
      window.StatSniper.HideDataset(event.target || event.srcElement);
    });
  ...
  HideDataset: function (element) {
    switch (String(element.id)) {
      case "processor-rectangle": {
        this.processorTriangle.style.animation = this.chart.getDatasetMeta(0)
          .hidden
          ? "fade-in-triangle 0.5s forwards"
          : "fade-out-triangle 0.5s forwards";

        this.chart.getDatasetMeta(0).hidden = this.chart.getDatasetMeta(0)
          .hidden
          ? false
          : true;
        break;
      ...
      case "ram-rectangle": {
        this.ramTriangle.style.animation = this.chart.getDatasetMeta(1).hidden
          ? "fade-in-triangle 0.5s forwards"
          : "fade-out-triangle 0.5s forwards";

        this.chart.getDatasetMeta(1).hidden = this.chart.getDatasetMeta(1)
          .hidden
          ? false
          : true;
        break;
      ...
      case "storage-rectangle": {
        this.storageTriangle.style.animation = this.chart.getDatasetMeta(2)
          .hidden
          ? "fade-in-triangle 0.5s forwards"
          : "fade-out-triangle 0.5s forwards";

        this.chart.getDatasetMeta(2).hidden = this.chart.getDatasetMeta(2)
          .hidden
          ? false
          : true;
        break;
  ...
  FormatLabels: function (labelArray, usageData) {
    ...
    switch (usageDataString.length) {
      case 1: {
        ...
        labelArray[0].style.color =
          this.html.getAttribute("theme") == "light"
            ? "rgba(188, 188, 188, 1)"
            : "rgba(121, 121, 121, 1)";
        labelArray[1].innerHTML = 0;
        labelArray[1].style.color =
          this.html.getAttribute("theme") == "light"
            ? "rgba(188, 188, 188, 1)"
            : "rgba(121, 121, 121, 1)";
        labelArray[2].innerHTML = usageDataString[0];
        labelArray[2].style.color =
          this.html.getAttribute("theme") == "light"
            ? "rgba(0, 0, 0, 1)"
            : "rgba(255, 255, 255, 1)";
        break;
